...
6. The website reads `/data/regimes.json` from the Worker. Pages Functions and the MCP Worker use a `REGIME_DATA_SERVICE` Service Binding to the same updater Worker, so internal Cloudflare requests cannot fall through to the checked-in static file. The static file in the Pages build remains an initial client-side fallback only.
...
- `GET /api/export`: compact export generated by the Pages Function from the active D1 snapshot through the updater Service Binding.
- `POST /mcp`: MCP tools read the active D1 snapshot through the updater Service Binding.
